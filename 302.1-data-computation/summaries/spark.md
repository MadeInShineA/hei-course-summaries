...
```mermaid
graph TB
    Client["Client"] -->|"Metadata Query<br/>(Q1, Q18)"| Master["Master<br/>(Metadata Only)"]
    Master -->|"Chunk Locations"| Client
    Client -->|"Direct Data Access"| Chunkservers["Chunkservers<br/>(64MB Chunks, 3 Replicas: Q2)"]
    style Client fill:#ef444420,stroke:#ef4444,stroke-width:2px
    style Master fill:#3b82f620,stroke:#3b82f6,stroke-width:2px
    style Chunkservers fill:#10b98120,stroke:#10b981,stroke-width:2px
```
...
```mermaid
graph TB
    Client["Client"] -->|"Metadata<br/>(Q7)"| NameNode["NameNode<br/>(Namespace/Blocks)"]
    NameNode -->|"Locations"| Client
    Client -->|"Direct I/O"| DataNodes["DataNodes<br/>(128MB Blocks, Replicas: Q9)"]
    style Client fill:#ef444420,stroke:#ef4444,stroke-width:2px
    style NameNode fill:#3b82f620,stroke:#3b82f6,stroke-width:2px
    style DataNodes fill:#10b98120,stroke:#10b981,stroke-width:2px
```
...
```mermaid
flowchart TD
    Input["Input Splits<br/>(Logical, Aligned: Q10, Q24)"] --> Map["Map<br/>(Local Disk: Q3–Q5)"]
    Map --> Shuffle["Shuffle<br/>(Reducers Pull: Q25,<br/>Costly: Q22)"]
    Shuffle --> Reduce["Reduce<br/>(HDFS Output: Q4, Q8)"]
    style Input fill:#3b82f620,stroke:#3b82f6,stroke-width:2px
    style Map fill:#10b98120,stroke:#10b981,stroke-width:2px
    style Shuffle fill:#f59e0b20,stroke:#f59e0b,stroke-width:2px
    style Reduce fill:#06b6d420,stroke:#06b6d4,stroke-width:2px
```
...
```mermaid
graph LR
    Source["Source<br/>(Boundary: Q17)"] --> Narrow["Narrow Transforms<br/>(Q19)"]
    Narrow --> Wide["Wide / Shuffle<br/>(Q13, Q17)"]
    Wide --> Action["Action<br/>(Triggers Execution)"]
    style Source fill:#3b82f620,stroke:#3b82f6,stroke-width:2px
    style Narrow fill:#10b98120,stroke:#10b981,stroke-width:2px
    style Wide fill:#f59e0b20,stroke:#f59e0b,stroke-width:2px
    style Action fill:#8b5cf620,stroke:#8b5cf6,stroke-width:2px
```
...
```mermaid
graph LR
    Storage["Storage Layer<br/>HDFS / Object Stores<br/>(Q6–7, Q28)"] --> Compute["Compute Layer<br/>MapReduce / Spark<br/>(Q8, Q28)"]
    Compute --> Formats["Data Formats<br/>Parquet / ORC<br/>(Q16, Q28)"]
    Storage --> SLURM["Orchestration<br/>SLURM<br/>(Q20)"]
    SLURM --> Compute
