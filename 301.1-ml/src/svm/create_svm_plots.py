...
for i, (C, gamma) in enumerate([(0.1, 0.01), (1, 0.01), (100, 0.01), (0.1, 10), (1, 10), (100, 10)]):
    # Create non-linear data
    ...
    # Plot dataset
    ax.scatter(X_complex[:, 0], X_complex[:, 1], c=y_complex, cmap=plt.cm.coolwarm, s=50)
...
plt.tight_layout()
plt.savefig("res/svm/svm_c_and_gamma_parameter_effect.png", dpi=150, bbox_inches='tight')
plt.show()
...
# Create a plot showing the effect of the epsilon parameter in SVM regression
from sklearn.svm import SVR

# Create sample regression data
X_eps, y_eps = make_regression(n_samples=100, n_features=1, noise=10, random_state=42)
...
for i, (eps, title) in enumerate(zip(eps_values, titles_eps)):
    ax = sub[i // 2, i % 2]
    
    # Fit regression model with different epsilon values
    svr = SVR(kernel='rbf', C=100, gamma=0.1, epsilon=eps)
    svr.fit(X_eps, y_eps)
    
    # Plot the regression
    X_plot = np.linspace(X_eps.min(), X_eps.max(), 300).reshape(-1, 1)
    y_plot = svr.predict(X_plot)

    ax.scatter(X_eps, y_eps, color='black', label='Data', alpha=0.6)
    ax.plot(X_plot, y_plot, color='red', label='SVR')
    ax.fill_between(X_plot.ravel(), 
                    svr.predict(X_plot) - eps, 
                    svr.predict(X_plot) + eps, 
                    color='red', alpha=0.2, label=f'Epsilon tube (ε={eps})')
...
plt.tight_layout()
plt.savefig("res/svm/svm_epsilon_parameter_effect.png", dpi=150, bbox_inches='tight')
plt.show()
...
for i, (title, model, X_data, y_data, variant_type) in enumerate(variants):
    ...
    if variant_type == 'classification':
        ...
        # Plot dataset
        ax.scatter(X_data[:, 0], X_data[:, 1], c=y_data, cmap=plt.cm.coolwarm, s=50, alpha=0.7)
        ax.set_xlabel("Feature 1")
        ax.set_ylabel("Feature 2")
        ...
        if "Epsilon" in title:
            ax.fill_between(X_plot.ravel(), 
                            ...
                            model.predict(X_plot) + model.epsilon, 
                            color='red', alpha=0.2, label=f'Epsilon tube (ε={model.epsilon})')
        ...
        if hasattr(model, 'support_'):
            ...
            ax.scatter(X_data[sv_indices], y_data[sv_indices], 
